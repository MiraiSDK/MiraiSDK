...
@interface OPGLESContext : CGContext
{
}
              ...
              drawable: (Drawable) w
{
  ...
  if (!ret) {
    ...
    GLXContext gl_ctx;
    cairo_device_t *device = cairo_glx_device_create(d,gl_ctx);
    cairo_content_t content;
    target = cairo_gl_surface_create(device,content,wa.width,wa.height);
...
{
  [super setSize: size];
    cairo_gl_surface_set_size(cairo_get_target(self->ct), size.width, size.height);

}

- (void) flushSurface
{
    XFlush(cairo_gl_surface_get_display(cairo_get_target(self->ct)));
}
@end

CGContextRef OPGLESContextCreate(Display *display, Drawable drawable)
{
  return [[OPGLESContext alloc] initWithDisplay: display
                                      drawable: drawable];
}
